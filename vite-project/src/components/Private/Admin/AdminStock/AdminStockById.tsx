...
const AdminStockById = () => {
  ...
  useEffect(() => {
    const getAdminStockById = async () => {
      try {
        ...
        setamount(res?.data?.adminstock?.purchasePrice);
      } catch (err) {
        if (axios.isAxiosError(err)) {
          toast.error(err.response?.data?.message || "Something went wrong");
        } else {
  ...
  const handleUpdateAdminStock = async (
    e: React.MouseEvent<HTMLButtonElement>,
  ) => {
    e.preventDefault();
    const adminstockdata: adminStockData = {
      product: productId,
      stock: stock,
  ...
  return (
    <div className="w-full">
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row">
        ...
        <button
          type="button"
          onClick={handleUpdateAdminStock}
          disabled={Loading}
        ...
        >
          {Loading ? "Creating..." : "Create Admin Stock"}
        </button>
      </div>
    </div>
  );
